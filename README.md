...
right-click → `Settings...`から、以下のグループに分けて外観・挙動をカスタマイズできます。
...
Stability Focused モードを選択すると、アートワーク未登録アルバムを常に表示する設定と、表示品質が自動的に Low に固定されます（負荷を抑えるための安全側の挙動です）。
...
A. 現時点では Columns UI 専用です。Default UI への対応は今後の予定に含まれています。
